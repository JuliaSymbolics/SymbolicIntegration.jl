data = [
    ...
    (integrand = ((21 + 3x)^5)*((1 / (4 + x))^7), result = (-27//2)*(7+x)^6/(4+x)^6, integration_var = x) #1_1_1_2_4 TODO here integrating (21 + 3x)^5) / (4 + x)^7 would not work because of pattern mathcing
    (integrand = (2+3x)^(1//2)*(4-6x)^(1//2), result = (2-3x)^(3//2)*x*(2+3x)^(3//2)/sqrt(2) + (6/sqrt(2))*x*sqrt(2-3x)*sqrt(2+3x) + 4*sqrt(2)*asin(3x/2), integration_var = x) #1_1_1_2_5 gets applied wtice TODO check why 1_1_1_2_8 doesnt get applied
