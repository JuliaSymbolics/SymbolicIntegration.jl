...
# ("1_1_3_1_11",
# @smrule ∫(((~a) + (~!b)*(~x)^(~n))^(~p),(~x)) => !contains_var((~x), (~a), (~b)) && igtQ((~n), 0) && ((~p) < -1) && (isa(2*(~p), Integer) || (~n) == 2 && isa(4*(~p), Integer) || (~n) == 2 && isa(3*(~p), Integer) || denominator((~p) + 1/(~n)) < denominator((~p))) ? -(~x)*((~a) + (~b)*(~x)^(~n))^((~p) + 1)⨸((~a)*(~n)*((~p) + 1)) + ((~n)*((~p) + 1) + 1)⨸((~a)*(~n)*((~p) + 1))*∫(((~a) + (~b)*(~x)^(~n))^((~p) + 1), (~x)) : nothing)
("1_1_3_1_12",
