...
("1_1_1_2_6",
@smrule ∫(1/(((~a) + (~!b)*(~x))^(3//2)*((~c) + (~!d)*(~x))^(3//2)),(~x)) =>
        !contains_var((~x), (~a), (~b), (~c), (~d)) &&
        eqQ((~b)*(~c) + (~a)*(~d), 0) ?
...
("1_1_1_2_15",
@smrule ∫(1/(((~a) + (~!b)*(~x))^(9//4)*((~c) + (~!d)*(~x))^(1//4)),(~x)) =>
        !contains_var((~x), (~a), (~b), (~c), (~d)) &&
        eqQ((~b)*(~c) + (~a)*(~d), 0) &&
        negQ((~a)^2*(~b)^2) ?
...
("1_1_1_2_17",
@smrule ∫(1/(((~a) + (~!b)*(~x))^(5//4)*((~c) + (~!d)*(~x))^(1//4)),(~x)) =>
        !contains_var((~x), (~a), (~b), (~c), (~d)) &&
        eqQ((~b)*(~c) + (~a)*(~d), 0) &&
        negQ((~a)^2*(~b)^2) ?
...
("1_1_1_2_21",
@smrule ∫(1/(sqrt((~a) + (~!b)*(~x))*sqrt((~c) + (~!d)*(~x))),(~x)) =>
        !contains_var((~x), (~a), (~b), (~c), (~d)) &&
        eqQ((~a) + (~c), 0) &&
        eqQ((~b) - (~d), 0) &&
        ((~a) > 0) ?
...
@smrule ∫(1/(sqrt((~!a) + (~!b)*(~x))*sqrt((~!c) + (~!d)*(~x))),(~x)) =>
        ...
        ((~b)*(~c) - (~a)*(~d) > 0) &&
        ((~b) > 0) ?
2⨸sqrt((~b))* substitute(∫(1⨸sqrt((~b)*(~c) - (~a)*(~d) + (~d)*(~x)^2), (~x)), (~x) => sqrt((~a) + (~b)*(~x))) : nothing)

# ("1_1_1_2_24",
# @smrule ∫(1/(((~!a) + (~!b)*(~x))*((~!c) + (~!d)*(~x))^(1//3)),(~x)) =>
#         !contains_var((~x), (~a), (~b), (~c), (~d)) &&
#         posQ(((~b)*(~c) - (~a)*(~d))/(~b)) ?
# With[{(~q) = rt(((~b)*(~c) - (~a)*(~d))⨸(~b), 3)}, -log((~a) + (~b)*(~x))⨸(2*(~b)*(~q)) - 3⨸(2*(~b)*(~q))*substitute(∫(1⨸((~q) - (~x)), (~x)), (~x) => ((~c) + (~d)*(~x))^(1⨸3)) + 3⨸(2*(~b))* Subst[∫(1⨸((~q)^2 + (~q)*(~x) + (~x)^2), (~x))) : nothing)
# 
# ("1_1_1_2_25",
# @smrule ∫(1/(((~!a) + (~!b)*(~x))*((~!c) + (~!d)*(~x))^(1//3)),(~x)) =>
#         !contains_var((~x), (~a), (~b), (~c), (~d)) &&
...
# 
# ("1_1_1_2_26",
# @smrule ∫(1/(((~!a) + (~!b)*(~x))*((~!c) + (~!d)*(~x))^(2//3)),(~x)) =>
#         !contains_var((~x), (~a), (~b), (~c), (~d)) &&
#         posQ(((~b)*(~c) - (~a)*(~d))/(~b)) ?
# With[{(~q) = rt(((~b)*(~c) - (~a)*(~d))⨸(~b), 3)}, -log((~a) + (~b)*(~x))⨸(2*(~b)*(~q)^2) - 3⨸(2*(~b)*(~q)^2)*substitute(∫(1⨸((~q) - (~x)), (~x)), (~x) => ((~c) + (~d)*(~x))^(1⨸3)) - 3⨸(2*(~b)*(~q))* Subst[∫(1⨸((~q)^2 + (~q)*(~x) + (~x)^2), (~x))) : nothing)
# 
# ("1_1_1_2_27",
# @smrule ∫(1/(((~!a) + (~!b)*(~x))*((~!c) + (~!d)*(~x))^(2//3)),(~x)) =>
#         !contains_var((~x), (~a), (~b), (~c), (~d)) &&
...
# 
# ("1_1_1_2_28",
# @smrule ∫(1/(((~!a) + (~!b)*(~x))^(1//3)*((~!c) + (~!d)*(~x))^(2//3)),(~x)) =>
#         !contains_var((~x), (~a), (~b), (~c), (~d)) &&
#         !eqQ((~b)*(~c) - (~a)*(~d), 0) &&
#         posQ((~d)/(~b)) ?
...
#         intlinearQ((~a), (~b), (~c), (~d), (~m), (~n), (~x)) ?
# With[{(~p) = Denominator[(~m))}, (~p)⨸(~b)* substitute(∫((~x)^((~p)*((~m) + 1) - 1)*((~c) - (~a)*(~d)⨸(~b) + (~d)*(~x)^(~p)⨸(~b))^(~n), (~x)), (~x) => ((~a) + (~b)*(~x))^(1⨸(~p)))) : nothing)
# 
# ("1_1_1_2_33",
# @smrule ∫(((~!b)*(~x))^(~m)*((~c) + (~!d)*(~x))^(~n),(~x)) =>
...
# 
# ("1_1_1_2_40",
# @smrule ∫(((~!a) + (~!b)*(~u))^(~!m)*((~!c) + (~!d)*(~u))^(~!n),(~x)) =>
#         !contains_var((~x), (~a), (~b), (~c), (~d), (~m), (~n)) &&
#         Symbolics.linear_expansion((~u), (~x))[3] &&
#         !eqQ(Coefficient[(~u), (~x), 0), 0] ?
# 1⨸Symbolics.coeff((~u), (~x) ^ 1)* substitute(∫(((~a) + (~b)*(~x))^(~m)*((~c) + (~d)*(~x))^(~n), (~x)), (~x) => (~u)) : nothing)
# 
