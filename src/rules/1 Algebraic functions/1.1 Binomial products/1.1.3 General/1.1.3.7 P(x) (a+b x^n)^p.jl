...
ext_coeff((~Pq), (~x), (~q))*(~x)^(exponent_of((~Pq), (~x)) - (~n) + 1)*((~a) + (~b)*(~x)^(~n))^((~p) + 1)⨸((~b)*(exponent_of((~Pq), (~x)) + (~n)*(~p) + 1)) + 1⨸((~b)*(exponent_of((~Pq), (~x)) + (~n)*(~p) + 1))* ∫(expand_to_sum( (~b)*(exponent_of((~Pq), (~x)) + (~n)*(~p) + 1)*((~Pq) - ext_coeff((~Pq), (~x), (~q))*(~x)^exponent_of((~Pq), (~x))) - (~a)*ext_coeff((~Pq), (~x), (~q))*(exponent_of((~Pq), (~x)) - (~n) + 1)*(~x)^(exponent_of((~Pq), (~x)) - (~n)), (~x))*((~a) + (~b)*(~x)^(~n))^(~p), (~x)) : nothing)
