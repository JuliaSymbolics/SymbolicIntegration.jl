...
Int[1/(a_ + b_.*x_^n_ + c_.*x_^n2_), x_Symbol] := With[{q = Rt[a/c, 2], r = Rt[2*q - b/c, 2]}, 1/(2*c*q*r)*Int[(r - x^(n/2))/(q - r*x^(n/2) + x^n), x] + 1/(2*c*q*r)*Int[(r + x^(n/2))/(q + r*x^(n/2) + x^n), x]] /; FreeQ[{a, b, c}, x] && EqQ[n2, 2*n] && NeQ[b^2 - 4*a*c, 0] && IGtQ[n/2, 0] && NegQ[b^2 - 4*a*c]
