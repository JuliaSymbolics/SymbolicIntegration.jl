...
Int[(a_ + b_.*sin[c_. + d_.*x_])^n_, x_Symbol] := -b*Cos[c + d*x]*(a + b*Sin[c + d*x])^(n - 1)/(d*n) + 1/n* Int[(a + b*Sin[c + d*x])^(n - 2)* Simp[a^2*n + b^2*(n - 1) + a*b*(2*n - 1)*Sin[c + d*x]], x] /; FreeQ[{a, b, c, d}, x] && NeQ[a^2 - b^2, 0] && GtQ[n, 1] && IntegerQ[2*n]
...
Int[(a_ + b_.*sin[c_. + d_.*x_])^n_, x_Symbol] := -b*Cos[ c + d*x]*(a + b*Sin[c + d*x])^(n + 1)/(d*(n + 1)*(a^2 - b^2)) + 1/((n + 1)*(a^2 - b^2))* Int[(a + b*Sin[c + d*x])^(n + 1)* Simp[a*(n + 1) - b*(n + 2)*Sin[c + d*x]], x] /; FreeQ[{a, b, c, d}, x] && NeQ[a^2 - b^2, 0] && LtQ[n, -1] && IntegerQ[2*n]
