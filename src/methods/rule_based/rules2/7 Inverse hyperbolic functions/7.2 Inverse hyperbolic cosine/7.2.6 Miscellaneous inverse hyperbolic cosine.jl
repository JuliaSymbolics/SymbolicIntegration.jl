...
("7_2_6_4",
:(((~!e) + (~!f)*(~x))^(~!m)*((~!A) + (~!B)*(~x) + (~!C)*(~x)^2)^ (~!p)*((~!a) + (~!b)*acosh((~c) + (~!d)*(~x)))^(~!n)) => :(
...
("7_2_6_19",
:(acosh(sqrt(1 + (~!b)*(~x)^2))^(~!n)/sqrt(1 + (~!b)*(~x)^2)) => :(
    !contains_var((~b), (~n), (~x)) ?
