...
1⨸(~n)*int_and_subst((~x)^(simplify(((~m) + 1)⨸(~n)) - 1)*((~d) + (~e)*(~x)^((~r)⨸(~n)))^ (~q)*((~a) + (~b)*log((~c)*(~x)))^(~p),  (~x), (~x), (~x)^(~n), "3_1_4_25") : nothing))
...
("3_1_4_29",
:(((~f) + (~!g)*(~x))^(~!m)*((~d) + (~!e)*(~x))^(~q)*((~!a) + (~!b)*log((~!c)*(~x)^(~!n)))^ (~!p)) => :(
