...
:(log((~!c)*(~x))/((~d) + (~!e)*(~x))) => :(
    ...
    eq((~e) + (~c)*(~d), 0) ?
-1⨸(~e)*PolyLog.reli(2, 1 - (~c)*(~x)) : nothing))
...
("3_1_3_17",
:(((~!a) + (~!b)*log((~!c)*(~x)^(~!n)))/(sqrt((~d1) + (~!e1)*(~x))* sqrt((~d2) + (~!e2)*(~x)))) => :(
